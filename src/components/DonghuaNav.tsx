...
const items: { key: Route['name']; label: string; icon: any; route: Route }[] = [
  { key: 'donghua', label: 'Donghua', icon: HomeIcon, route: { name: 'donghua' } },
...
export function DonghuaNav() {
  ...
  return (
    <div className="fixed bottom-1 left-0 right-0 z-40 safe-bottom px-3 pb-3 pointer-events-none">
      <div
      ...
      >
        <div className="flex items-center justify-between">
          {items.map(it => {
            ...
            const Icon = it.icon;
            return (
              <button key={it.key} onClick={() => navigate(it.route)} className="flex-1 flex flex-col items-center justify-center gap-0.7 py-1.5 rounded-2xl relative">
