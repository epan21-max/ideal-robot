  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = Array.isArray(data) ? data : (data?.genreList || data?.genres || data?.list || data?.data || []);
        const mapped = list.map((g: any) => ({
          name: g.title || g.name || (typeof g === 'string' ? g : 'Unknown'),
          slug: g.genreId || g.slug || g.endpoint || '',
        }));
    ...
    <div className="px-4 pt-20">
      <div className="mb-5 animate-fade-in-up">
        <div className="flex items-center gap-1.5 text-xs uppercase tracking-widest text-[var(--muted)] mb-1">
          <TagIcon size={12} /> Browse
        </div>
        <h1 className="text-3xl font-extrabold tracking-tight">Genres</h1>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 gap-3 stagger">
          {genres.map((g, i) => (
            <button
              key={g.name + i}
              onClick={() => g.slug && navigate({ name: 'genre-detail', slug: g.slug, genreName: g.name })}
            ...
            >
              ...
              <div className="absolute -top-4 -right-4 opacity-30 text-white group-hover:opacity-40 group-hover:rotate-12 transition-all">
                <TagIcon size={64} />
              </div>
