...
import { ChevronLeftIcon, ChevronRightIcon, PlayIcon, ClockIcon, ChevronDownIcon, TvIcon, DownloadIcon, ShareIcon, CheckIcon } from '../components/Icon';

export function EpisodePage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const d = await api.episode(slug);
        if (!m) return;
        setData(d);
        const url = d?.defaultStreamingUrl || d?.stream_url || d?.video_url || d?.url || '';
        setStreamUrl(url);
        setActiveQuality('default');
  ...
  const animeSlug = data.animeId || data.anime_slug || data.anime?.slug || '';
  const releaseTime = data.releaseTime;
  const prev = data.prevEpisode?.episodeId || data.previous_episode?.slug;
  const next = data.nextEpisode?.episodeId || data.next_episode?.slug;
  const qualities: any[] = data.server?.qualities || [];
  const downloads: any[] = data.downloadUrl?.qualities || data.download_urls || [];

  return (
    <div className="pb-32">
      {/* Top bar */}
      <div
        className="sticky z-30 px-4 pt-3 pb-2 liquid-glass-strong"
        style={{ top: 0, paddingTop: 'max(env(safe-area-inset-top), 12px)' }}
      >
        <div className="flex items-center gap-3">
          <button onClick={back} className="w-10 h-10 rounded-full bg-[var(--surface)] flex items-center justify-center shrink-0">
      ...
      <div className="px-4 mt-4 animate-fade-in-up">
        ...
        {releaseTime && (
          <div className="inline-flex items-center gap-1 text-xs text-[var(--muted)] mt-1">
            <ClockIcon size={11} /> {releaseTime}
          </div>
        )}
        <div className="mt-2 flex items-center justify-between gap-2">
          {animeSlug ? (
            <button
              onClick={() => navigate({ name: 'detail', slug: animeSlug })}
        ...
        <div className="grid grid-cols-2 gap-2 mt-4">
          <button
            disabled={!prev}
            onClick={() => prev && navigate({ name: 'episode', slug: prev })}
            className="liquid-glass rounded-2xl py-3 px-3 inline-flex items-center justify-center gap-1.5 text-sm font-semibold disabled:opacity-40"
          >
            <ChevronLeftIcon size={14} /> Previous
          </button>
          <button
            disabled={!next}
            onClick={() => next && navigate({ name: 'episode', slug: next })}
            className="liquid-glass rounded-2xl py-3 px-3 inline-flex items-center justify-center gap-1.5 text-sm font-semibold disabled:opacity-40"
          >
        ...
        <div className="grid grid-cols-2 gap-2 mt-6 items-start">
            ...
            <div className="liquid-glass rounded-2xl overflow-hidden shadow-xl border border-black/5 dark:border-white/5 transition-all duration-300">
              ...
              {openServers && qualities.length > 0 && (
                <div className="p-2.5 pt-1 border-t border-black/5 dark:border-white/5 space-y-2.5 animate-slide-up bg-black/5 dark:bg-white/5">
                  {qualities.map((q: any, qi: number) => (
                    <div key={qi} className="space-y-1">
                      <div className="text-[10px] font-black text-[var(--accent)] uppercase tracking-wider">
                        {q.title}
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {(q.serverList || []).map((s: any, si: number) => {
                          const isActive = activeQuality === q.title && activeServer === s.serverId;
                          return (
                            <button
                              key={si}
                              onClick={async () => {
                                setActiveQuality(q.title);
                                setActiveServer(s.serverId);
                                try {
                                  const res = await fetch(`https://www.sankavollerei.web.id${s.href}`);
                                  const json = await res.json();
                                  const url = json?.data?.url || json?.url || json?.data?.embed || '';
          ...
          <div className="col-span-1">
            <div className="liquid-glass rounded-2xl overflow-hidden shadow-xl border border-black/5 dark:border-white/5 transition-all duration-300">
              ...
              >
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[9px] text-[var(--muted)]">{downloads.length || 0} Links</div>
                  </div>
                </div>
              ...
              {openDownloads && downloads.length > 0 && (
                <div className="p-2.5 pt-1 border-t border-black/5 dark:border-white/5 space-y-2.5 animate-slide-up bg-black/5 dark:bg-white/5">
                  {downloads.map((d: any, i: number) => {
                    const quality = d.title || d.resolution || d.quality || `Option ${i + 1}`;
                    const size = d.size;
                    const urls: any[] = d.urls || d.links || [];
                    return (
                      <div key={i} className="space-y-1">
                        <div className="flex items-center justify-between text-[10px] font-black text-emerald-500 uppercase tracking-wider">
                          <span>{quality}</span>
                          {size && <span className="text-[8px] opacity-70 font-mono">{size}</span>}
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {urls.map((u: any, j: number) => (
                            ...
                            >
                              {(u.title || u.provider || u.name || 'Link').trim()}
                            </a>
                          ))}
                        </div>
                      </div>
